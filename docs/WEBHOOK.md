...
- `PENDING` 이벤트는 수집 시점의 endpoint에 묶입니다. endpoint가 바뀌면 자동으로 새 서버에 보내지 않고 `FAILED`로 전환합니다.
- 새 endpoint를 확인한 뒤 **실패 재시도**를 누르면 실패 이벤트를 현재 endpoint에 명시적으로 다시 연결합니다.
- 같은 endpoint의 token만 교체하면 아직 시작되지 않은 요청은 새 token을 사용합니다.
